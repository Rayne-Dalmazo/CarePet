...
btnSalvarRemedio?.addEventListener("click", async () => {
   ...
   const dataToSend = {
    ...
    try {
       const r = await fetch("https://back-production-7df8.up.railway.app/api/remedio", {
    method: "POST",
...
btnSalvarVacina?.addEventListener("click", async () => {
    ...
    try {
        const r = await fetch("https://back-production-7df8.up.railway.app/api/vacina", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
btnSalvarExame?.addEventListener("click", async () => {
    ...
    try {
        const r = await fetch("https://back-production-7df8.up.railway.app/api/exame", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(dataToSend)
        });
